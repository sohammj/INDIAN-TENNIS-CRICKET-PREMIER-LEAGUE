...
export async function seedDevData() {
  ...
  for (let i = 0; i < matchPairs.length; i++) {
    ...
    const match = await prisma.match.create({
      data: {
        tournamentId: tournament.id,
        teamAId: createdTeams[a as number].id,
        teamBId: createdTeams[b as number].id,
    ...
    const matchPlayers = [
      ...createdPlayers.slice((a as number) * 3, (a as number) * 3 + 3),
      ...createdPlayers.slice((b as number) * 3, (b as number) * 3 + 3),
    ];
